...
const TradingInterface = () => {
  ...
  const [clickedActions, setClickedActions] = React.useState([]) // Store clicked actions

  const sendToBackend = async (action) => {
    const updatedActions = [...clickedActions, action] // Append new action to list
    setClickedActions(updatedActions) // Update state
    ...
    try {
      const response = await fetch('http://127.0.0.1:5000/ui_analyse', {
        ...
        body: JSON.stringify({ actions: updatedActions }), // Send updated list
      })
    ...
    <div className={containerClasses}>
      ...
      >
          ...
          <div className="relative">
            ...
            >
              <Wallet color="#4BB4B3" strokeWidth={2} size={16} />
              <span className="font-bold text-[#4BB4B3]">10,000.00 USD</span>
              <ChevronDown size={16} />
            </button>
            ...
            {showBalancePopup && (
              ...
              >
                ...
                <button
                  className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 transition-colors"
                  onClick={() => sendToBackend('withdrawal')}
                >
                  Withdrawal
                </button>
                {/* TRANSFER */}
                <button
                  className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 transition-colors"
                  onClick={() => sendToBackend('transfer')}
                >
                  Transfer
                </button>
              </div>
